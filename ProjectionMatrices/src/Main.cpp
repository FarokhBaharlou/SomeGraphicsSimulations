...
void setBasicProjectionMatrix(const float& angleOfView, const float& near, const float& far, Matrix4f& M)
{
    // do some work here
...
void multPointMatrix(const Vec3f& in, Vec3f& out, const Matrix4f& M)
{
    //out = in * Mproj;
...
void DrawBasic()
{
    ...
    setBasicProjectionMatrix(angleOfView, near, far, Mproj);
    ...
    // export to image
    std::ofstream ofs;
    ofs.open("output\\ResultBasic.ppm");
...
void gluPerspective(const float& angleOfView, const float& imageAspectRatio, const float& n, const float& f, float& b, float& t, float& l, float& r)
{
...
void glFrustum(const float& b, const float& t, const float& l, const float& r, const float& n, const float& f, Matrix4f& M)
{
...
void glOrtho(const float& b, const float& t, const float& l, const float& r, const float& n, const float& f, Matrix4f& M)
{
    // set OpenGL perspective projection matrix
    M[0][0] = 2 / (r - l);
    M[0][1] = 0;
...
void glOrthoDraw()
{
    uint32_t imageWidth = 512, imageHeight = 512;
    Matrix4f Mproj;
    Matrix4f worldToCamera = { 0.95424, 0.20371, -0.218924, 0, 0, 0.732087, 0.681211, 0, 0.299041, -0.650039, 0.698587, 0, -0.553677, -3.920548, -62.68137, 1 };
    ...
    // export to image
    std::ofstream ofs;
    ofs.open("output\\glOrthoResult.ppm");
    ofs << "P5\n" << imageWidth << " " << imageHeight << "\n255\n";
    ofs.write((char*)buffer, imageWidth * imageHeight);
    ofs.close();
    delete[] buffer;
