...
public:
    ...
    TriangleMesh(const Matrix4f& o2w, const uint32_t nfaces, const std::unique_ptr<uint32_t[]>& faceIndex, const std::unique_ptr<uint32_t[]>& vertsIndex, const std::unique_ptr<Vec3f[]>& verts, std::unique_ptr<Vec3f[]>& normals, std::unique_ptr<Vec2f[]>& st) : Object(o2w), numTris(0)
    {
...
int main(int argc, char** argv)
{
    ...
    options.width = 1024;
    options.height = 747;
    options.cameraToWorld = Matrix4f(-0.972776, 0, -0.231748, 0, -0.114956, 0.8683, 0.482536, 0, 0.201227, 0.49604, -0.844661, 0, 6.696465, 22.721296, -30.097976, 1);

    TriangleMesh* mesh1 = loadPolyMeshFromFile("geometry\\backdrop.geo", Matrix4f::kIdentity);
    if (mesh1 != nullptr)
    {
        mesh1->type = kDiffuse;
    ...
    TriangleMesh* mesh3 = loadPolyMeshFromFile("geometry\\cylinder.geo", Matrix4f::kIdentity);
    if (mesh3 != nullptr)
    {
    ...
    TriangleMesh* mesh4 = loadPolyMeshFromFile("geometry\\pen.geo", Matrix4f::kIdentity);
    if (mesh4 != nullptr)
    {
        mesh4->type = kPhong;
        mesh4->albedo = 0.18;
    ...
    */
    Matrix4f l2w(11.146836, -5.781569, -0.0605886, 0, -1.902827, -3.543982, -11.895445, 0, 5.459804, 10.568624, -4.02205, 0, 0, 0, 0, 1);
    ...
    options.width = 1024;
    options.height = 747;
    options.cameraToWorld = Matrix4f(0.707107, 0, -0.707107, 0, -0.331295, 0.883452, -0.331295, 0, 0.624695, 0.468521, 0.624695, 0, 28, 21, 28, 1);

    TriangleMesh* mesh = loadPolyMeshFromFile("geometry\\plane.geo", Matrix4f::kIdentity);
    if (mesh != nullptr)
    {
        mesh->type = kDiffuse;
        mesh->albedo = 0.18;
    ...
    options.width = 1024;
    options.height = 747;
    options.cameraToWorld = Matrix4f(0.999945, 0, 0.0104718, 0, 0.00104703, 0.994989, -0.0999803, 0, -0.0104193, 0.0999858, 0.994934, 0, -0.978596, 17.911879, 75.483369, 1);

    TriangleMesh* mesh = loadPolyMeshFromFile("geometry\\glasses.geo", Matrix4f::kIdentity);
    if (mesh != nullptr)
    {
    ...
    TriangleMesh* mesh1 = loadPolyMeshFromFile("geometry\\backdrop1.geo", Matrix4f::kIdentity);
    if (mesh1 != nullptr)
    {
        mesh1->type = kDiffuse;
        mesh1->albedo = 0.18;
    ...
    Matrix4f l2w(0.95292, 0.289503, 0.0901785, 0, -0.0960954, 0.5704, -0.815727, 0, -0.287593, 0.768656, 0.571365, 0, 0, 0, 0, 1);
#elif 0
    // aliasing example
    options.fov = 36.87;
    options.width = 1024;
