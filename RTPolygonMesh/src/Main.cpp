...
public:
    ...
    TriangleMesh(const uint32_t nfaces, const std::unique_ptr<uint32_t[]>& faceIndex, const std::unique_ptr<uint32_t[]>& vertsIndex, const std::unique_ptr<Vec3f[]>& verts, std::unique_ptr<Vec3f[]>& normals, std::unique_ptr<Vec2f[]>& st) : numTris(0)
    {
...
bool trace(const Vec3f& orig, const Vec3f& dir, const std::vector<std::unique_ptr<Object>>& objects, float& tNear, uint32_t& index, Vec2f& uv, Object** hitObject)
{
    *hitObject = nullptr;
...
void render(const Options& options, const std::vector<std::unique_ptr<Object>>& objects, const uint32_t& frame)
{
    ...
    char buff[256];
    sprintf_s(buff, "output\\Result.ppm", frame);
    std::ofstream ofs;
    ofs.open(buff);
...
int main()
{
    ...
    Matrix4f tmp = Matrix4f(0.707107, -0.331295, 0.624695, 0, 0, 0.883452, 0.468521, 0, -0.707107, -0.331295, 0.624695, 0, -1.63871, -5.747777, -40.400412, 1);
    options.cameraToWorld = tmp.inverse();
    options.fov = 50.0393;
    #if 1 
    std::vector<std::unique_ptr<Object>> objects;
    TriangleMesh* mesh = loadPolyMeshFromFile("input\\cow.geo");
